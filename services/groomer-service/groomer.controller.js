...
// Create Groomer
const createGroomer = asyncHandler(async (req, res) => {
...
// Get All Groomers
const getAllGroomers = asyncHandler(async (req, res) => {
...
// Get Groomer By ID
const getGroomerById = asyncHandler(async (req, res) => {
  const { id } = req.params;
...
// Update Groomer
const updateGroomer = asyncHandler(async (req, res) => {
...
/**--------- */
const updateGroomerProfileByGroomer = asyncHandler(async (req, res) => {
...
// Delete Groomer
const deleteGroomer = asyncHandler(async (req, res) => {
  const { id } = req.params;
...
const updateGroomerStatus = asyncHandler(async (req, res) => {
...
const createVaccineSchedule = asyncHandler(async (req, res) => {
  const {
    pet,
    vaccineName,
    batchNumber = null,
  ...
  const requiredFields = { pet, vaccineName, vaccinationDate };
  const missingFields = Object.keys(requiredFields).filter(
    (field) => !requiredFields[field] || requiredFields[field] === "undefined"
  );
  ...
  // Check pet exists
  const petExists = await PetRegistration.findById(pet);
  if (!petExists) {
    return res.status(404).json(new ApiError(404, "Pet not found"));
  }

  // Check vaccine exists
  const vaccineExists = await Vaccine.findById(vaccineName);
  if (!vaccineExists) {
    return res.status(404).json(new ApiError(404, "Vaccine not found"));
  }

  const vaccineSchedule = await VaccineSchedule.create({
    pet,
    vaccineName,
    batchNumber,
    vaccinationDate,
  ...
  return res
    .status(201)
    .json(new ApiResponse(201, vaccineSchedule, "Vaccine Schedule created successfully"));
});

// Update vaccine schedule
const updateVaccineSchedule = asyncHandler(async (req, res) => {
  const { id } = req.params;
  ...
  // If pet is being updated, verify existence
  if (pet) {
    const petExists = await PetRegistration.findById(pet);
    if (!petExists) {
      return res.status(404).json(new ApiError(404, "Pet not found"));
  ...
  // If vaccineName is being updated, verify existence
  if (vaccineName) {
    const vaccineExists = await Vaccine.findById(vaccineName);
    if (!vaccineExists) {
      return res.status(404).json(new ApiError(404, "Vaccine not found"));
  ...
  if (!updatedVaccineSchedule) {
    return res.status(404).json(new ApiError(404, "Vaccine Schedule not found"));
  }

  return res
    .status(200)
    .json(
      new ApiResponse(200, updatedVaccineSchedule, "Vaccine Schedule updated successfully")
    );
});

// Get vaccine schedule by id
const getVaccineScheduleById = asyncHandler(async (req, res) => {
  const { id } = req.params;
  ...
  return res
    .status(200)
    .json(new ApiResponse(200, vaccineSchedule, "Vaccine Schedule fetched successfully"));
});

// Get all vaccine schedules
const getAllVaccineSchedules = asyncHandler(async (req, res) => {
  // Optionally add filtering, pagination, etc.

  const vaccineSchedules = await VaccineSchedule.find()
    .populate("pet")
    .populate("vaccineName");

  return res
    .status(200)
    .json(new ApiResponse(200, vaccineSchedules, "Vaccine Schedules fetched successfully"));
});

// Delete vaccine schedule
const deleteVaccineSchedule = asyncHandler(async (req, res) => {
  const { id } = req.params;
  ...
  const vaccineSchedule = await VaccineSchedule.findByIdAndDelete(id);
  ...
  return res
    .status(200)
    .json(new ApiResponse(200, "Vaccine Schedule deleted successfully"));
});
    ...
    const proximityCheck = await Address.aggregate([
      {
        ...
        {
    $match: { distance: { $lte: PROXIMITY_THRESHOLD } }
  },
      { $limit: 1 }
    ]).session(session);
